    ...
    def _pre_fit(self, kwargs):
        ...
        for key, value in list(self._X.items()):
            assert key in list(net.blobs.keys()), (
                'data key "{}" has no corresponding network blob in {}'.format(
...
class ResultExtractor(Monitor):  # pylint: disable=R0903
    ...
    def _pre_fit(self, kwargs):
        ...
        #     print("Blob", self._layer_name, "was found and will be logged.")
        # else:
        #     print("Blob", self._layer_name, "was NOT found!")
        self._init = True
...
class JSONLogger(Monitor):  # pylint: disable=R0903
    ...
    * ``iter``\*,
    ...
    def __init__(self, path, name, logging, offset=0):
        """See class documentation."""
        import json
        ...
        self.offset = offset
        if offset > 0:
            name += '_continuing_from_' + str(offset)
            # TODO: better save content from old file and combine with new data
    ...
    def _post(self, phase_name, kwargs):  # pylint: disable=C0111
        if phase_name not in self._logging:
            return
        # print('what we want to log:', self._logging)
        # print('what we can log:', kwargs.keys())
        for key in self._logging[phase_name]:
            if key in kwargs:
                numIters = kwargs['iter'] + self.offset
                # print(key, kwargs[key], 'was logged')
                self.dict[phase_name].append({'NumIters': numIters,
                                              key: kwargs[key]})
    ...
    * ``batch_size``\*.
    ...
    def __init__(self,
        ...
        _LOGGER.info('Setting up checkpointing with name prefix %s every ' +
                     '%d iterations, starting from an offset of %s.',
                     name_prefix, iterations, offset)
